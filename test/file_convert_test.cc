...
std::string output_aac = "/Users/zhanghao/code/cpp/media_fusion/test/test_file/output.aac";
std::string output_mp3 = "/Users/zhanghao/code/cpp/media_fusion/test/test_file/output.mp3";

// 测试文件是否能够打开
TEST(file_convert, open_file_test){
    file_convert file;
    file.file_open(input_mp4);
    file.file_open(input_wav);
}
...
TEST(file_convert, video_to_audio_test){
    file_convert file;
    file.file_open(input_mp4);
    file.video_to_audio(output_aac);
}
...
TEST(file_convert, audio_convert_test){
    file_convert file;
    file.file_open(input_wav);
    file.video_to_audio(output_mp3);
}